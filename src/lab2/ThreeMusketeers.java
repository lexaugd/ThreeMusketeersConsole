...
public class ThreeMusketeers {
    ...
    protected void move(final Agent agent) { // TODO
        Move copyOfMove;
        if (isHumansPlaying()) {//if human
            copyOfMove = new Move(agent.getMove());
            board.move(copyOfMove);
    ...
    private void undoMove() { // TODO
        if(moves.size()!=0){
            board.undoMove(moves.remove(moves.size()-1)); //send copy of a move
